...
@Component({
  ...
  template: `
          ...
          <div class="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-lg p-6 border border-blue-200">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div class="text-center">
                ...
                <p class="text-sm text-gray-600">{{(budgetOverview.remaining || 0) >= 0 ? 'Remaining' : 'Over Budget'}}</p>
              </div>
            </div>
    ...
    <app-confirmation-modal
      [isVisible]="showClearBudgetModal"
      title="Clear Budget"
      message="Are you sure you want to clear your budget? This will remove all budget tracking and reset your budget to $0."
      confirmText="Clear Budget"
      cancelText="Cancel"
      type="danger"
      icon="delete"
      (confirmed)="confirmClearBudget()"
      (cancelled)="cancelClearBudget()">
    </app-confirmation-modal>
  `
})
export class ProfileEditComponent implements OnInit, OnDestroy {
  ...
  private updateBudgetWithTransactionData(): void {
    if (this.budgetOverview && this.transactions.length > 0) {
      ...
      console.log('Profile Edit - Updated budget overview with transaction data:', this.budgetOverview);
    }
  }
  ...
  onSubmit() {
    ...
    this.budgetService.updateBudget(budgetData)
      .pipe(takeUntil(this.destroy$))
      .subscribe({
        next: (updatedBudget: MonthlyBudget) => {
